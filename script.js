...
async function foo1() {
    try {
        for (var i = 1; i <= 10; i++) {
            var empty = [];
            let data2 = await fetch(`https://www.anapioficeandfire.com/api/books/${i}`);
            let data3 = await data2.json();
            var cc = "";
            for (var j = 0; j < data3.authors.length; j++) {
                cc = cc + (data3.authors[j]) + ",";

            }
            var cc1 = "";
            for (var k = 0; k < 5; k++) {
                var len = data3.characters.length;
                if (len >= 5) {
                    let data4 = await fetch(`${data3.characters[k]}`);
                    let data5 = await data4.json();
                    cc1 = cc1 + (data5.name) + ",";
                }
                else {
                    for (var m = 0; m < len; m++) {
                        let data6 = await fetch(`${data3.characters[k]}`);
            ...
            empty.push(i, data3.name, data3.isbn, data3.numberOfPages, cc.slice(0, -1), data3.publisher, data3.released, cc1.slice(0, -1));
